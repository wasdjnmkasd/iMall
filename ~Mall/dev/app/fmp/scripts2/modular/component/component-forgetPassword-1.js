...
/** 模块组件
 *    @detail:    forgetPassword组件_1
 *    @return:    can.Component
 *    @author:    林鹏腾
 *    @date:      2017.8.16
 */
define(["text!modular/template/template-forgetPassword-1.mustache"], function(template){
    ...
    return can.Component.extend({
        tag: "component-forgetpassword-1",
        template: template,
        helpers: {
        },
        scope: {
            ...
            validateConfirmPassword: function(password, confirmPassword){
                return confirmPassword.trim()!=="" && password === confirmPassword && (/^.{6,12}$/gi).test(password);
            }
        },
        events: {
            "inserted": function(){
            ...
            ".component-forgetPassword-content>div>.iconRight touchstart": function(node){
                var $node = $(node);
                var $input = $node.prev();
            ...
            ".component-forgetPassword-content>div>.iconRight touchend": function(node){
                var $node = $(node);
                var $input = $node.prev();
            ...
            ".component-forgetPassword-forgetPasswordBtn touchend": function(){
                var that = this;
                var $element = that.element;
                ...
                if(account_state && validation_state && password_state && confirmPassword_state){
                    that.scope.attr("forgetPasswordBtn_state", "");
                }else{
                    that.scope.attr("forgetPasswordBtn_state", "state_error");
                }
            }
        }
    })
