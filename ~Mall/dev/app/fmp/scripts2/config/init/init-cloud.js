...
(function(API){
    ...
    if(!API.jsUtil.mustache){
        Object.defineProperty(API.jsUtil, 'mustache', {
            configurable: false,
            enumerable: true,
            writable: false,
            value: {
                getContent: function(cont, type){
                    var tempCont = typeof cont==='function'? cont(): cont;
                    var tempType = typeof type==='function'? type(): type;
                    if((typeof tempCont === tempType) || tempType === undefined){
                        return typeof tempCont === "string" && tempCont !== "null"? tempCont.trim(): tempCont !== "null"? tempCont: "";
                    }
                    else if(tempType === "function"){
                        return function(){};
                    }
                    else if(tempType === "string"){
                        return typeof tempCont === "number" && tempCont === tempCont? tempCont.toString(): "";
                    }
                    else if(tempType === "number"){
                        return typeof tempCont === "string" && parseFloat(tempCont) == tempCont? parseFloat(tempCont.trim()): 0;
                    }
                    else if(tempType === "boolean"){
                        return tempCont === "true";
                    }
                    else{
                        return null
                    }
                },
                getDepContent: function(cont, type){
                    var tempCont = typeof cont==='function'? cont(): cont;
                    var tempType = typeof type==='function'? type(): type;
                    if((typeof tempCont === tempType) || tempType === undefined){
                        ...
                        else if(typeof tempCont === "string"){
                            return tempCont !== "null"? tempCont.trim(): "";
                        }
                        else if(typeof tempCont === "object"){
                            return tempCont && typeof tempCont.attr === "function"? tempCont.attr(): tempCont;
                        }
                        else{
                            return tempCont;
                        }
    ...
    if(!API.jsUtil.modular){
        Object.defineProperty(API.jsUtil, 'modular', {
            configurable: false,
            enumerable: true,
            writable: false,
            value: {
                newModule: function(code, area, data){
                    var index = 1;
                    var count = "001";
                    var page = API.jsEvent.page;
                    var codes = API.jsModular.codes;
                    var $div = document.createElement("div");
                    var $area = document.querySelector(area||"body");
                    if(!code || area === "none" || !codes[code] || !codes[code].md || !codes[code].cls){
                        return null
                    }else{
                        var Module = codes[code].md;
                        var clsName = codes[code].cls;
                        var clsAttr = "[class^='"+clsName+"-']";
                        while(index<=document.querySelectorAll(clsAttr).length){
                            var clsCount = "[class='"+clsName+"-"+count+"']";
                            if(document.querySelectorAll(clsCount).length===0) {
                                break;
                            }else{
                                count = (""+(1000+(++index))).substr(1);
                            }
                        }
                        $area.appendChild($div);
                        $div.classList.add(clsName+"-"+count);
                        $div.setAttribute("pageCode", ++page.code);
                        return new Module("."+clsName+"-"+count, data||{});
                    }
                },
                floorQueue: function(data, type, isCover){
                    var newArr = [];
                    if(isPlainObject(data) && isPureArray(data[type])){
                        each(data[type], function(i, obj){
                            if(isPlainObject(obj) && isPlainObject(obj.floorCode)){
                                each(obj.floorCode, function(id, count){
                                    var arr = newArr[id] = newArr[id]||[];
                                    if(arr.length < count){
                                        arr[arr.length] = obj;
                                        return false;
                                    }
                    ...
                    return isCover!==false? data[type] = newArr: newArr;
                },
                showModules: function(mdArr){
                    var modular = API.jsUtil.modular;
                    var modules = API.jsModular.modules;
                    if(isPureArray(mdArr)){
                        each(mdArr, function(index, mdArr){
                            if(isPureArray(mdArr)){
                                each(mdArr, function(num, obj){
                                    var isObject = isPlainObject(obj);
                                    if(isObject && obj.code){
                                        var module = modular.newModule(obj.code, obj.area, obj);
                                        obj.role? modules[obj.role] = module: null;
                                    }
                                })
                            }
                        });
    ...
    if(!API.jsModel.path){
        Object.defineProperty(API.jsModel, 'path', {
            ...
            value: {
                 ...
                 *    nav-1         模块数据    GOODS_NAV_MAP_QUERY_NODE -------
                 *    nav-1         模块数据    GOODS_NAV_FIRST_QUERY_NODE -----
                 ...
                 *    grade         grade信息  GRADE_GRADEBO_QUERY_REDIS ------- welfareType
                ...
                ORDER_SHOPPINGCART_DELETE:   {
                    host:         "",
                    path:         "/ordercenter/1.0/order/shoping-cart/{gradeId}/{userId}/{ids}?platformSource={platformSource}",
                    method:       "DELETE",
                    context:      "",
                    contType:     "application/x-www-form-urlencoded;charset=UTF-8",
                    reqDataType:  "",
    ...
    init: function(API){
        ...
        var newWelfare =  API.jsUtil.url.getParam("welfareVip");
        ...
        var oldWelfare =  API.jsUtil.storage.getItem("welfareVip");
        ...
        var welfOption =  { "welfareVip": newWelfare };
        var authOption =  { "authId": newAuthId, "userId": newUserId };
        var dumpOption =  [ "shopId", "userId", "authId", "openId", "welfareVip"];
        ...
        if (newWelfare) API.jsUtil.storage.setItem(welfOption);
        ...
        return {
            ...
            welfareVip: !signErr && getWelfare === 'v:' + getShopId + '-' + getUserId,
    ...
    open: function(API){
        window.capi = {
            get: function(rm){ return rm && this.remove() || API; },
            remove: function(){ delete window.entity; delete window.capi; }
        }
    },
    jump: function(API){
        var jumpUrl =      '';
