...
define(["text!modular/template/template-searchHistory-1.mustache"], function(template){
    ...
    return can.Component.extend({
        ...
        helpers: {
            ...
            returnHotSearch: function(hotSearch, options){
                var tHotSearch = api.jsUtil.mustache.getDepContent(hotSearch, "object");
                return $.isArray(tHotSearch) && tHotSearch.length>0 ?
                    options.fn(this.context): options.inverse(this.context);
            }
        },
        scope: {
